...
//        });
    ...
    private void showNotification(String songName, boolean isPlaying) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            String channelId = "MusicPlayerChannel";
        ...
        // Create the intent for the notification buttons
        ...
        Intent nextIntent = new Intent(this, NotificationActionReceiver.class)
                .setAction("NEXT");
        Intent previousIntent = new Intent(this, NotificationActionReceiver.class)
                .setAction("PREVIOUS");

        // Create pending intents for the notification buttons
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(this, "MusicPlayerChannel")
            ...
            if (action != null) {
                if (action.equals("PLAY_PAUSE")) {
                    if (paused) {
                        ...
                        buttonMain.setImageResource(R.drawable.ic_pause);
                        paused = false;
                        showNotification(songPlay.getText().toString(), true); // Update notification for playing state
                    } else {
                        nowPlaying.pause();
                        buttonMain.setImageResource(R.drawable.ic_play);
                        paused = true;
                        showNotification(songPlay.getText().toString(), false); // Update notification for paused state
                    }
                } else if (action.equals("NEXT")) {
                    int currentIndex = trackNames.indexOf(songName);
                    if (currentIndex < trackNames.size() - 1) {

                        String nextSong = trackNames.get(currentIndex + 1);
                        playSong(nextSong);
